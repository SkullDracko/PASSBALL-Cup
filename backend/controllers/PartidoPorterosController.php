...
class PartidoPorterosController
{
    ...
    public function registrar(array $params): void
    {
        ...
        $atajadas = $this->numero($body['atajadas'] ?? null, 'atajadas');
        $goles = $this->numero($body['goles_recibidos'] ?? null, 'goles_recibidos');
        $this->verificarPorteroConvocado($partidoId, $jugadorId);
        try {
            $stmt = $this->pdo->prepare('INSERT INTO partido_estadisticas_portero (partido_id, jugador_id, atajadas, goles_recibidos) VALUES (?, ?, ?, ?)');
            $stmt->execute([$partidoId, $jugadorId, $atajadas, $goles]);
        } catch (PDOException $e) {
            jsonResponse(false, [], ['error' => 'El portero ya tiene estadísticas registradas para este partido'], 409);
        }
    ...
    public function actualizar(array $params): void
    {
        ...
        $this->verificarRegistro($partidoId, $jugadorId);
        $body = jsonBody();
        $campos = [];
        $values = [];
        foreach (['atajadas', 'goles_recibidos'] as $campo) if (array_key_exists($campo, $body)) {
            $campos[] = "{$campo} = ?";
            $values[] = $this->numero($body[$campo], $campo);
        }
        if (!$campos) jsonResponse(false, [], ['error' => 'No hay campos válidos para actualizar'], 400);
        $values[] = $partidoId;
        $values[] = $jugadorId;
        $stmt = $this->pdo->prepare('UPDATE partido_estadisticas_portero SET ' . implode(', ', $campos) . ' WHERE partido_id = ? AND jugador_id = ?');
        $stmt->execute($values);
    ...
    public function eliminar(array $params): void
    {
        ...
        $partidoId = $this->id($params, 'partidoId');
        $jugadorId = $this->id($params, 'jugadorId');
        $stmt = $this->pdo->prepare('DELETE FROM partido_estadisticas_portero WHERE partido_id = ? AND jugador_id = ?');
        $stmt->execute([$partidoId, $jugadorId]);
        if ($stmt->rowCount() === 0) $this->noEncontrado();
        jsonResponse(true, ['mensaje' => 'Estadística de portero eliminada correctamente']);
    }
    ...
    private function verificarPorteroConvocado(int $partidoId, int $jugadorId): void
    {
        $stmt = $this->pdo->prepare('SELECT id FROM partido_convocados WHERE partido_id = ? AND jugador_id = ? AND posicion = "portero"');
        $stmt->execute([$partidoId, $jugadorId]);
        if (!$stmt->fetch()) jsonResponse(false, [], ['error' => 'El jugador debe estar convocado como portero'], 422);
    }
    private function verificarRegistro(int $partidoId, int $jugadorId): void
    {
        $stmt = $this->pdo->prepare('SELECT id FROM partido_estadisticas_portero WHERE partido_id = ? AND jugador_id = ?');
        $stmt->execute([$partidoId, $jugadorId]);
        if (!$stmt->fetch()) $this->noEncontrado();
    }
    private function noEncontrado(): void
    {
        jsonResponse(false, [], ['error' => 'Estadística de portero no encontrada'], 404);
    }
}
